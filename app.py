...
# --- CARICAMENTO DATI (Funzione ottimizzata) ---
...
# --- SEZIONE 1: SIDEBAR (Inserimento e Cancellazione) ---
with st.sidebar:
    ...
    if st.button("🗑️ CANCELLA ULTIMA RIGA"):
        try:
            ...
            if r.status_code == 200:
                st.warning("Ultima riga eliminata!")
                st.cache_data.clear()
                st.rerun()
            else: st.error("Errore cancellazione")
        except Exception as e: st.error(f"Errore: {e}")

# --- SEZIONE 2: VISUALIZZAZIONE DATI ---
try:
    # Aggiungiamo un parametro casuale all'URL per evitare la cache di Google
    ...
    scelta = st.selectbox("Filtra storico:", opzioni)
    ...
    if 'Delta' in df_filtrato.columns and 'Data' in df_filtrato.columns:
        ...
        linea = base.mark_line(color="#888888", opacity=0.7)
        punti = base.mark_circle(size=80).encode(
            color=alt.condition("abs(datum.Delta) <= 0.05", alt.value("#29b09d"), alt.value("#ff4b4b")),
            tooltip=['Data', 'Delta', 'Tipo percorso']
        )
        zero = alt.Chart(pd.DataFrame({'y': [0]})).mark_rule(color='white', strokeDash=[3,3]).encode(y='y')
        st.altair_chart((linea + punti + zero).properties(height=300).interactive(bind_y=False), use_container_width=True)
